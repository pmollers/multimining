...
# Приступим?
  ## Как мне видится весь процесс:
  - [x] поднимаем ноду монеты, настройки и конфиги зависят от монеты, благо для каждой есть гит. Проводим синхронизацию, либо заливаем бутстрап монеты, если есть, получаем новый кошелек и аккаунт.
  - [ ] поднимаем базу данных, настраиваем обновление бирж по нашей монете, грузим по крону биды и объемы. Нужен скрипт, который будет выдергивать нужную монету, а не весь API разом по всем нужным биржам. Настройка биржи должна быть минимально простой.
  - [ ] поднимаем стратум сервер. Пока не определился со скриптом, но как минимум он должен быть простым и поддерживать любой алгоритм.
  - [ ] веб морда. Пока планирую взять готовое на php. Для личных нужд она по сути вообще не нужна, но как то прияно видеть некоторые процессы. Скорее всего это будет yiimp, но раздербаненный на части.
  - [ ] скрипт переключения портов и валют на основе профитности валюты. Простой скрипт, должен анализировать все имеющиеся валюты с разбиением по алгоритму, иначе смысл!? Порт снаружи для майнера меняться не должен, должен меняться порт на сервере между портами стратумов монет, т.е. если профит валюта А - порт такой-то, если профит валюта Б - порт такой то. И производить переназначение внешнего порта на внутренний. Явно прокси, пока не ясно какой.
  ...
  Первым делом, решено использовать крон для обновлений. Сейчас базу валют с бирж тянет php по крону, грузит в mysql. Убираем php путем написания простого скрипта. Проверено на импорте сложности валюты в базу mysql. Файл [test.sh](test.sh) Сложность грузится в базу простым скриптом, пока решен вопрос с блокэксплоером.
